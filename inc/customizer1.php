...
function stackdesign_customize_register( $wp_customize ) {
	...
	$stackdesign_get_font_size_schemes = stackdesign_get_font_size_schemes();
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			$wp_customize, 'background_color', array(
				'label'       => __( 'Background color', 'stackdesign' ),
				'description' => __( 'Applied arbitrary color to background color.', 'stackdesign' ),
				'section'     => 'colors',
			)
		)
	);
...
//Font setting section and settings and controls

$wp_customize->add_section( 'stackdesign_font_setting' , array(
    'title'      => __( 'Font setting', 'stackdesign' ),
    'priority'   => 50,
) );

// Add color scheme setting and control.
	$wp_customize->add_setting(
		'font_size_setting', array(
			'default'           => 'default',
			'sanitize_callback' => 'stackdesign_sanitize_color_scheme',
			'transport'         => 'postMessage',
		)
	...
	$wp_customize->add_control(
		'font_size_setting', array(
			'label'    => __( 'Font size setting', 'stackdesign' ),
			'section'  => 'stackdesign_font_setting',
			'type'     => 'select',
			'choices'  =>stackdesign_get_font_size_setting_body_choices(),
			'priority' => 1,
		)
	);
...
function stackdesign_get_font_size_schemes() {
	/**
	 * Filter the color schemes registered for use with Stack Design.
	 *
	 * The default schemes include 'default', 'dark', 'yellow', 'pink', 'purple', and 'blue'.
	...
	return apply_filters(
		'stackdesign_color_schemes', array(
			...
			'medium' => array(
				'label'  => __( 'Medium', 'stackdesign' ),
				'sizes' => array(
                    '12px',
				),
			),
			'small' => array(
				'label'  => __( 'Small', 'stackdesign' ),
				'sizes' => array(
                    '12px',
				),
			),
			'large' => array(
				'label'  => __( 'Large', 'stackdesign' ),
				'sizes' => array(
                    '12px',
				),
			),
...
if ( ! function_exists( 'stackdesign_get_font_size_setting_body_choices' ) ) :
	/**
	 * Returns an array of color scheme choices registered for Stack Design.
	 *
	 * @since Stack Design 0.9
	 ...
	 * @return array Array of color schemes.
	 */
	function stackdesign_get_font_size_setting_body_choices() {
		$font_size_setting=stackdesign_get_font_size_schemes();
		$font_size_setting_control_options = array();

		foreach ( $font_size_setting as $font_size_setting => $value ) {
			$font_size_setting_control_options[$font_size_setting ] =$value['label'];
		}

		return $font_size_setting_control_options;
	}
endif; // //Font setting section and settings and controls
...
function tcx_customizer_css() {
    ?>
    <style type="text/css">
        a { color: <?php echo get_theme_mod( 'background_color' ); ?>; }
    </style>
    <?php
}
